...
char* RC4Encrypt::Encrypt(const char* szSource)
{
	char szPassWord[] = ENCRYPT_PWD;
...
char*  RC4Encrypt::Decrypt(const char* szSource)
{
	char szPassWord[] = ENCRYPT_PWD;
...
int  RC4Encrypt::RC4(const unsigned char* data, int data_len, const unsigned char* key, int key_len, unsigned char* out, int* out_len)
{
...
int  RC4Encrypt::GetKey(const unsigned char* pass, int pass_len, unsigned char* out)
{
...
void  RC4Encrypt::swap_byte(unsigned char* a, unsigned char* b)
{
...
char*  RC4Encrypt::ByteToHex(const unsigned char* vByte, const int vLen)
{
	if(!vByte)
	   return NULL;
...
unsigned char*  RC4Encrypt::HexToByte(const char* szHex)
{
	if(!szHex)
	   return NULL;
